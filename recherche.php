...
function get(){
  $req = 'SELECT MARQUE.MAR_NOM, UTILISATEUR.UTI_NOM,UTILISATEUR.UTI_PRENOM,VILLE.VIL_NOM,VILLE.VIL_CP, INS_NOM, INS_DESCRIPTION,INS_PHOTO,UTI_PHOTO,CAT_FAMILLE FROM `INSTRUMENT` as instru
  ...
  function send($req){
    ...
    for ($i=0;$i<sizeof($new);$i++) {
      echo(utf8_encode($new[$i]["MAR_NOM"] . '|' . $new[$i]["UTI_NOM"] . '|' . $new[$i]["UTI_PRENOM"] . '|' . $new[$i]["VIL_NOM"] . '|' . $new[$i]["INS_NOM"]
        . '|' . $new[$i]["INS_DESCRIPTION"] . '|' . $new[$i]["INS_PHOTO"] . '|' . $new[$i]["UTI_PHOTO"] . '|' . $new[$i]["CAT_FAMILLE"] . '|' . $new[$i]["VIL_CP"] . '//||//'));
    }
  }
?>
